/*
	Prusa_MK25SMMU2.cpp - Printer definition for the Prusa MK25S w/MMU2
...
#include "printers/Prusa_MK25S_13.h"  // for Prusa_MK3S
...
void Prusa_MK25SMMU2::OnVisualTypeSet(const std::string &type)
{
	if (type != "none")
	{
		Prusa_MK2_13::OnVisualTypeSet(type);
...
std::pair<int,int> Prusa_MK25SMMU2::GetWindowSize()
{
	auto prSize = Prusa_MK25S_13::GetWindowSize();
	prSize.second +=50;
	return prSize;
}

void Prusa_MK25SMMU2::Draw()
{
	glPushMatrix();
		Prusa_MK25S_13::Draw();
...
void Prusa_MK25SMMU2::OnMMUFeed(struct avr_irq_t *, uint32_t value)
{
	float fVal;
